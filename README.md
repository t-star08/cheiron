...
- [Cheiron (ケイロン, ケイローン)](#cheiron-ケイロン-ケイローン)
  - [v0.0.0 -> v1.0.0 (2021/05/14)](#v000---v100-20210514)
  - [目次](#目次)
  - [インストール](#インストール)
  - [使い方](#使い方)
  - [前書き](#前書き)
...
正規表現が使える (正規表現についての注意点は[こちら](#正規表現についての注意点))
...
このコマンドでは以下の点に注意が必要
1. `path/./to` のような正規表現をした際、"." は正規表現として認識されない
1. `path/.+/to` のような正規表現をした際、"path" 下に文字列としての ".+" という名前のディレクトリがある場合、".+" は正規表現として認識されない

「1.」については問題なため、[策](#正規表現のショートカット) を用意している
